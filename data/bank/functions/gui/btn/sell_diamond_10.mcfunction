...
scoreboard players remove @p[distance=..4,limit=1] bank_tmp2 10

# 成交（准备金不变）：账户 & 账本
execute if score @p[distance=..4,limit=1] bank_tmp2 matches 0.. run scoreboard players remove @p[distance=..4,limit=1] diamond 10
...
execute if score @p[distance=..4,limit=1] bank_tmp2 matches 0.. run scoreboard players remove #TD bank_total 10
...
# 提示
execute if score @p[distance=..4,limit=1] bank_tmp2 matches 0.. run tellraw @p[distance=..4,limit=1] [{"text":"[银行] 卖出 钻×10 成功 ｜余额：铜 ","color":"dark_red"},{"score":{"name":"@p","objective":"copper"}},{"text":"，钻 ","color":"gray"},{"score":{"name":"@p","objective":"diamond"}}]
