...
execute if score @p[distance=..4,limit=1] bank_tmp2 matches 0.. run scoreboard players add @p[distance=..4,limit=1] diamond 1
...
execute if score @p[distance=..4,limit=1] bank_tmp2 matches 0.. run scoreboard players add #TD bank_total 1

# 提示
execute if score @p[distance=..4,limit=1] bank_tmp2 matches 0.. run tellraw @p[distance=..4,limit=1] [{"text":"[银行] 买入 钻×1 成功 ｜余额：铜 ","color":"green"},{"score":{"name":"@p","objective":"copper"}},{"text":"，钻 ","color":"gray"},{"score":{"name":"@p","objective":"diamond"}}]
